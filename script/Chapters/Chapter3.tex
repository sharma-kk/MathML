...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
% We can see from equations \eqref{eq:bp_eq} that the expressions for derivatives with respect to weights and biases essentially remain the same when compared to equations for single-neuron network \ref{fig:sim_NN}.
% We see an addtional summation term in the expression for $\frac{\partial C}{\partial a_j^{(l)}}$. This is attributed to the fact that 
%%%%%%%%%%% UAT %%%%%%%%%%%%%%%%%%%%%
